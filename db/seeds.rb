...
Challenge::Scq.find_or_create_by!(
  date_when_available: Date.strptime('29/02/2024', '%d/%m/%Y'),
  name: "Main Function Return Type",
  difficulty: :MEDIUM,
...
question_array: [
  ...
  correct_answer_explanation: "The first element (*p) is incremented by 1, changing from 5 to 6. "
)
...
int main() {
    ...
    unsigned int b = 5;  // 0101 in binary
    unsigned int result;

    // Bit-wise AND operation
    result = a & b; // Should be 4 (0100 in binary)
    printf("a & b = %u\\n", result);
    ...
    result = a | b; // Should be 13 (1101 in binary)
    printf("a | b = %u\\n", result);
    ...
    for (int i = 1; i <= MAX_VALUE; i++) {
        switch(i) {
            case 1:
                ...
                break;
            case 3:
                sum += i * 2;  // Double the value of i and add to sum
                break;
            case 5:
                sum += i * 3;  // Triple the value of i and add to sum
                break;
            default:
                sum += i;  // Add i to sum for all other cases
                break;
        }
    }
    printf("Sum = %d\\n", sum);
...
question_array: [
  ...
  correct_answer_regex_array: ["^25$", "^2$"],
  correct_answer_explanation: "The sum is calculated as 1 (case 1) + 2 (default for 2) + 6 (double 3 for case 3) + 4 (default for 4) + 15 (triple 5 for case 5), which totals 25. The default case is executed for the values of i that are 2 and 4. "
)
...
question_array: [
  ...
  correct_answer_regex_array: ["^31$"],
  correct_answer_explanation: "With MAX_VALUE as 6, the additional loop iteration adds 6 to the previous total of 25, resulting in 31."
)
...
int main() {
    ...
    scanf("%s", b.material);  // Assuming input doesn't contain spaces
...
question_array: [
  ...
  correct_answer_regex_array: ["^int$", "^10$", "^50$"],
...
question_array: [
  ...
  correct_answer_regex_array: ["^%f$", "^15.500000$"],
  correct_answer_explanation: "The %f format specifier is used for floating-point numbers, which is the type of width. The printf function, when using %f, outputs floating-point numbers with six digits after the decimal by default."
)
...
question_array: [
  ...
  correct_answer_regex_array: ["^B$", "^5$"],
...
question_array: [
  ...
  correct_answer_regex_array: ["^B B B B B $", "^A A A $"],
  correct_answer_explanation: "Since c is 'B' and the loop prints it 5 times, the output is five 'B' characters separated by spaces. If num is 3, the condition num > 3 is false, so c remains 'A'. The loop executes three times, printing 'A A A '. This combines concepts of conditionals and loops."
)
