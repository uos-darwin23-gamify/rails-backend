<script>
	...
	function parseGroupsAndAnswers() {
		...
		challenge.first_group = document
			.getElementById('first-group')
			.value.split('\n')
			.filter(Boolean);
		challenge.second_group = document
			.getElementById('second-group')
			.value.split('\n')
			.filter(Boolean);
...
<form
	on:submit|preventDefault={submitChallenge}
...
>
	...
	<div>
		...
		<textarea id="overview" on:input={(e) => (challenge.question_overview = e.target.value)}
		></textarea>
	</div>
	...
	<div>
		<label for="correct-answers"
			>Correct Answers (Format: "index1,index2; index3,index4; etc.")</label
		>
	...
	<div>
		...
		<textarea
			id="explanation"
			on:input={(e) => (challenge.correct_answer_explanation = e.target.value)}
		></textarea>
	</div>
