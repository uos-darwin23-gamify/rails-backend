...
<script lang="ts">
	...
	import { ChallengeCategory } from '../ChallengeEditor.svelte';
...
int main() {
	...
	let name = '';
	let question_overview = '';
	let difficulty: (typeof challengeDifficulties)[0] = challengeDifficulties[0];
	let correct_answer_explanation = '';
	let type: (typeof challengeTypes)[0] = challengeTypes[0];
	let date_when_available: DateValue | undefined = new CalendarDate(
		new Date().getFullYear(),
		new Date().getMonth(),
		new Date().getDate()
	);
	$: isPlacementChallenge = date_when_available === undefined;

	let scqData: { answers: string[]; correct_answer: number } = { answers: [''], correct_answer: 0 };
				...
				<Tabs.Root value={isPlacementChallenge ? 'placement' : 'normal'}>
					<Tabs.List class="w-full overflow-x-auto">
						<Tabs.Trigger
							...
							value="normal"
							on:click={() =>
								(date_when_available = new CalendarDate(
									new Date().getFullYear(),
									new Date().getMonth(),
									new Date().getDate()
						...
						<Tabs.Trigger
							class="grow basis-1/2"
							value="placement"
							on:click={() => (date_when_available = undefined)}>Placement Challenge</Tabs.Trigger
						>
					</Tabs.List>
				</Tabs.Root>
				{#if errors !== null}
